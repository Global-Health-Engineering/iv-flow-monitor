...
#include "dripito_app.h" /* for DRIP_FACTOR_GTT_PER_ML */
...
static uint32_t mode_state       = 0; /* 0=show infused, 1=time, 2=drip_factor_gtt   */
...
static void ui_render_run(void)
{
    ...
    if (mode_state == 1) {
    	...
    	sprintf(l2, "Drops/ml %5u", DRIP_FACTOR_GTT_PER_ML);
    }
...
void ui_on_button(enum ui_button_e btn, bool long_press)
{
    ...
    case UI_RUN:
    	if (btn == BTN_MODE) {
    		adjust_drop_volume = 0;
        	mode_state = (mode_state + 1) % 3; /* three states */
        } else if (btn == BTN_MUTE && mode_state == 2 && DRIP_FACTOR_GTT_PER_ML < 999) { /* edit drop volume */
        	DRIP_FACTOR_GTT_PER_ML += 1;
        	adjust_drop_volume = 1;
